...
bool LRUReplacer::victim(frame_id_t* frame_id) {
    ...
    //有frame被移除
    *frame_id = LRUlist_.back();//被移除frame的id为列表中的最后一个元素
    LRUlist_.pop_back();//从列表中删除该frame
    LRUhash_.erase(*frame_id);//将该frame从哈希表中删除
...
void LRUReplacer::pin(frame_id_t frame_id) {
    ...
    if(LRUhash_.find(frame_id) != LRUhash_.end())//如果该页面存在的话
    {
        LRUlist_.erase(LRUhash_[frame_id]);//将该frame从count=0的列表中删除
...
void LRUReplacer::unpin(frame_id_t frame_id) {
    ...
    {
        LRUlist_.push_front(frame_id);//将该frame加入List的开头
